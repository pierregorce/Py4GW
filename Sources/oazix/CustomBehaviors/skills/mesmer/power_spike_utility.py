    ...
    def detect_casting_enemies(self) -> list[TargetingEnemyData]:
        targets = TargetingEnemy\
            ...
            condition_predicate=lambda enemy_data:
                GLOBAL_CACHE.Skill.Flags.IsSpell(Agent.GetCastingSkillID(enemy_data.agent_id)) 
                and enemy_data.interrupt_potential_score > 0 
                and TargetingCore().is_lock_key_available(self._get_lock_key(enemy_data.agent_id)), # only skills that are longer than 1s. too much changes to fail otherwise
            sort_asc_predicate=lambda enemy_data: 0 if enemy_data.is_caster else 1,
