...
class MirrorOfDisenchantmentUtility(CustomSkillUtilityBase):
    def __init__(self,
        event_bus: EventBus,
        ...
        super().__init__(
            event_bus=event_bus,
            skill=CustomSkill("Mirror_of_Disenchantment"),
    ...
    def _get_targets(self) -> list[TargetingEnemyData]:
        """Get enchanted enemies, prioritizing casters first."""
        targets = TargetingEnemy.create().get_enemies(
            ...
            sort_asc_predicate=lambda enemy_data: (0 if enemy_data.is_caster else 1, enemy_data.distance_from_player)
        )
