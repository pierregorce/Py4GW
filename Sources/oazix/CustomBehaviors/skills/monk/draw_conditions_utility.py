...
class DrawConditionsUtility(CustomSkillUtilityBase):
    def __init__(
        self,
        event_bus: EventBus,
        ...
        super().__init__(
            event_bus=event_bus,
            skill=CustomSkill("Draw_Conditions"),
    ...
    def _get_targets(self) -> list[TargetingAllyData]:
        return TargetingAlly.create().get_allies(
            ...
            condition_predicate=lambda ally_data: Agent.IsConditioned(ally_data.agent_id),
    ...
    def _execute(self, state: BehaviorState) -> Generator[Any, None, BehaviorResult]:
        ...
        if CustomBehaviorParty().get_shared_lock_manager().try_aquire_lock(lock_key) is False:
            yield
            return BehaviorResult.ACTION_SKIPPED
