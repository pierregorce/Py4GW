...
class FollowPathExecutor:
    ...
    def start(self, waypoints: list[tuple[float, float]]):
        if not waypoints:
            self.logger.information("FollowPathExecutor: No path provided")
            return
    ...
    def __create_generator(self, waypoints: list[tuple[float, float]]) -> Generator[None, None, None]:
        ...
        try:
            ...
            self.logger.information(f"generate_autopathing completed with {len(waypoints)} waypoints")
        ...
        if instance is None: 
            self.logger.information("FollowPathExecutor: No combat behavior instance available")
            return
        ...
        self.logger.information(f"FollowPathExecutor: Started movement with {len(self.current_path)} waypoints, generator={self.generator is not None}")
    ...
    def stop(self):
        ...
        self.logger.information("FollowPathExecutor: Stopped movement")
    ...
    def act(self):
        ...
        try:
            next(self.generator)
            self.logger.information(f"FollowPathExecutor: Movement step completed")
        except StopIteration:
            self.logger.information("FollowPathExecutor: Movement completed")
            self.stop()
        except Exception as e:
            self.logger.information(f"FollowPathExecutor: Movement error: {e}")
            self.stop()
    ...
    def on_progress(self, progress: float) -> None:
        ...
        self.logger.information(f"FollowPathExecutor: Progress {self.movement_progress}%")
