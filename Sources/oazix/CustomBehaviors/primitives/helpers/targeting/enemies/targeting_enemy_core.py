...
class TargetingEnemyCore:

    _instance = None  # Singleton instance

    def __new__(cls):
        if cls._instance is None:
            cls._instance = super(TargetingEnemyCore, cls).__new__(cls)
    ...
    def __build_combined_enemy_targets_key(
        ...
        allegiance_to_include: TargetingEnemyAllegiance,
        ...
        return f"combined|pos:{rounded[0]},{rounded[1]}|range:{within_range:.0f}|{leader_part}|{allegiance_part}|{aggressive_part}"

    # ----------------------------- Agent Data Builder -----------------------------

    @staticmethod
    def __build_sortable_agent_data(agent_id: int, source_pos: tuple[float, float]) -> TargetingEnemyData:
        """Build a TargetingEnemyData object for the given agent."""
        agent_pos = Agent.GetXY(agent_id)
        
        return TargetingEnemyData(
            agent_id=agent_id,
            ...
            enemy_and_ally_quantity_within_range=0,  # Computed separately if needed
            melee_optimised_aoe_score=0, # Computed separately if needed
            interrupt_potential_score=0, # Computed separately if needed
    ...
    def __get_leader_targets(self, leader_agent_id: int, within_range: float) -> list[TargetingEnemyData]:
        """
        ...
        :return: List of TargetingEnemyData for enemies within range of leader
        """
    ...
    def __get_aggressive_enemies_further(
        ...
        :return: List of TargetingEnemyData for aggressive enemies
        """
    ...
    def __get_enemies_by_distance(
        ...
        :return: List of TargetingEnemyData for enemies within range
        """
        all_enemies = AgentArray.GetEnemyArray()
    ...
    def get_combined_enemy_targets(
            ...
            allegiance_to_include: TargetingEnemyAllegiance = TargetingEnemyAllegiance.Enemy | TargetingEnemyAllegiance.Spirit | TargetingEnemyAllegiance.Pet | TargetingEnemyAllegiance.Minion,
        ...
        :return: Deduplicated list of TargetingEnemyData
        """
        cache_key = self.__build_combined_enemy_targets_key(
            source_pos, within_range, leader_agent_id, allegiance_to_include, include_aggressive_further
        )

        # Check cache first
        cached = MemoryCacheManager().get(cache_key)
        if cached is not None:
            return cast(list[TargetingEnemyData], cached)
        ...
        unique_agents: list[TargetingEnemyData] = []
        for agent in agents:
