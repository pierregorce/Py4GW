...
class AssassinVaettirFarm_Running_UtilitySkillBar(CustomBehaviorBaseUtility):
    ...
    def __init__(self, event_bus: EventBus):
        ...
        #core
        self.deadly_paradox_utility: CustomSkillUtilityBase = DeadlyParadoxUtility(event_bus=self.event_bus, score_definition=ScoreStaticDefinition(70), current_build=in_game_build, allowed_states=[BehaviorState.IN_AGGRO])
        ...
        self.shadow_form_utility: CustomSkillUtilityBase = ShadowFormUtility(event_bus=self.event_bus, score_definition=ScoreStaticDefinition(65), is_deadly_paradox_required=True,  renew_before_expiration_in_milliseconds=3500, current_build=in_game_build, allowed_states=[BehaviorState.IN_AGGRO])
        self.heart_of_shadow_utility: CustomSkillUtilityBase = HeartofShadowUtility(event_bus=self.event_bus, current_build=in_game_build, score_definition=ScoreStaticDefinition(50), mana_required_to_cast=20, allowed_states=[BehaviorState.IN_AGGRO, BehaviorState.CLOSE_TO_AGGRO, BehaviorState.FAR_FROM_AGGRO])

        # others
        self.dark_escape_utility: CustomSkillUtilityBase = KeepSelfEffectUpUtility(event_bus=self.event_bus, skill=CustomSkill("Dark_Escape"), current_build=in_game_build, score_definition=ScoreStaticDefinition(50), allowed_states=[BehaviorState.IN_AGGRO, BehaviorState.CLOSE_TO_AGGRO, BehaviorState.FAR_FROM_AGGRO])
