...
class ArmorOfUnfeelingUtility(CustomSkillUtilityBase):
    def __init__(self, skill: CustomSkill, current_build: list[CustomSkill], score_definition: ScoreStaticDefinition, spirit_refresh_state: SpiritRefreshState, allowed_states: list[BehaviorState] = [BehaviorState.IN_AGGRO, BehaviorState.CLOSE_TO_AGGRO]) -> None:
        super().__init__(skill=skill, in_game_build=current_build, score_definition=score_definition, allowed_states=allowed_states)
        self.spirit_refresh_state = spirit_refresh_state
    ...
    def _evaluate(self, current_state: BehaviorState, previously_attempted_skills: list[CustomSkill]) -> float | None:
        ...
        if self.spirit_refresh_state.spirits_have_been_recreated():
    ...
    def _execute(self, state: BehaviorState) -> Generator[Any, None, BehaviorResult]:
        ...
        if result is BehaviorResult.ACTION_PERFORMED:
            self.spirit_refresh_state.armor_of_unfeeling_refreshed()
        return result 