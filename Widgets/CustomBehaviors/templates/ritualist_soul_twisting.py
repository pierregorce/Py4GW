...
class RitualistSoulTwisting(CustomBehaviorBaseUtility):
    ...
    def __init__(self, cached_data: CacheData):
        ...
        self.shelter_utility: CustomSkillUtilityBase = ProtectiveSpiritUtility(skill=CustomSkill("Shelter"), current_build=in_game_build, score_definition=ScoreStaticDefinition(66), owned_spirit_model_id=SpiritModelID.SHELTER, spirit_refreshed=refresh_shelter)
        self.union_utility: CustomSkillUtilityBase = ProtectiveSpiritUtility(skill=CustomSkill("Union"), current_build=in_game_build, score_definition=ScoreStaticDefinition(65), owned_spirit_model_id=SpiritModelID.UNION, spirit_refreshed=refresh_union)
        self.displacement_utility: CustomSkillUtilityBase = ProtectiveSpiritUtility(skill=CustomSkill("Displacement"), current_build=in_game_build, score_definition=ScoreStaticDefinition(64), owned_spirit_model_id=SpiritModelID.DISPLACEMENT, spirit_refreshed=lambda: None)
        
        self.summon_spirit_utility: CustomSkillUtilityBase = SummonSpiritUtility(skill=CustomSkill("Summon_Spirits_luxon"), current_build=in_game_build, score_definition=ScoreStaticDefinition(95), 
            owned_spirits= [self.shelter_utility.owned_spirit_model_id, self.union_utility.owned_spirit_model_id, self.displacement_utility.owned_spirit_model_id])
        ...
        self.breath_of_the_great_dwarf_utility: CustomSkillUtilityBase = BreathOfTheGreatDwarfUtility(skill=CustomSkill("Breath_of_the_Great_Dwarf"), current_build=in_game_build, score_definition=ScorePerHealthGravityDefinition(0))
