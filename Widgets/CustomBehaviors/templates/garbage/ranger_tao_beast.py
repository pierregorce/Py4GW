from typing import List, Any, Generator, Optional, override
...
class RangerTaoBeast(CustomBehaviorBase):
    ...
    def _handle_far_from_aggro(self, cached_data: CacheData) -> Generator[Any | None, Any | None, None]:
        while True:
            yield from custom_behavior_helpers.Actions.cast_effect_before_expiration(self.together_as_one, time_before_expire=100)
            yield

    def _handle_close_to_aggro(self, cached_data: CacheData) -> Generator[Any | None, Any | None, None]:
        while True:
            yield from custom_behavior_helpers.Actions.cast_effect_before_expiration(self.together_as_one, time_before_expire=100)
            yield from custom_behavior_helpers.Actions.cast_effect_before_expiration(self.never_rampage_alone, time_before_expire=100)
            yield from custom_behavior_helpers.Actions.cast_effect_before_expiration(self.expert_focus, time_before_expire=2300)
            yield

    def _handle_in_aggro(self, cached_data: CacheData) -> Generator[Any | None, Any | None, None]:
        while True:

            result = yield from custom_behavior_helpers.Actions.cast_effect_before_expiration(self.together_as_one, time_before_expire=100)
            if result is BehaviorResult.ACTION_PERFORMED: continue

            result = yield from custom_behavior_helpers.Actions.cast_effect_before_expiration(self.never_rampage_alone, time_before_expire=100)
            if result is BehaviorResult.ACTION_PERFORMED: continue

            result = yield from custom_behavior_helpers.Actions.cast_effect_before_expiration(self.expert_focus, time_before_expire=2300)
            if result is BehaviorResult.ACTION_PERFORMED: continue

            if custom_behavior_helpers.Resources.get_player_absolute_energy() < 8: #never_rampage_alone cost 7 for 14 expertise
            ...
            if target_half_life is not None:
                ...
                result = yield from custom_behavior_helpers.Actions.cast_skill_to_target(self.needling_shot, target_agent_id=target_half_life)
                if result is BehaviorResult.ACTION_PERFORMED: continue # we loop to that skill

            result = yield from custom_behavior_helpers.Actions.cast_skill_to_lambda(self.scavenger_strike, select_target=
            lambda: custom_behavior_helpers.Targets.get_first_or_default_from_enemy_ordered_by_priority(
                within_range=Range.Spellcast,
            ...
            if current_target is not None:
                result = yield from custom_behavior_helpers.Actions.cast_skill_to_target(self.sundering_attack, target_agent_id=current_target)
                if result is BehaviorResult.ACTION_PERFORMED: continue
            ...
            yield from custom_behavior_helpers.Actions.auto_attack(current_target)
