...
class NecroBipResto(CustomBehaviorBase):
    ...
    def _handle_in_aggro(self, cached_data: CacheData) -> Generator[Any | None, Any | None, None]:
        while True:
            ...
            if custom_behavior_helpers.get_player_absolute_energy() > 15: # we keep mana for healing first
    ...
    def __try_great_dwarf_weapon(self) -> Generator[Any, Any, BehaviorResult]:
        ...
        result: BehaviorResult = yield from custom_behavior_helpers.cast_skill_to_lambda(
    ...
    def __healing_logic(self) -> Generator[Any, Any, BehaviorResult]:
        ...
        if first_member_with_x is not None and custom_behavior_helpers.Resources.is_spirit_exist(within_range=Range.Earshot):
            result = yield from custom_behavior_helpers.cast_skill_to_target(self.mend_body_and_soul, target_agent_id=first_member_damaged)
            if result is BehaviorResult.ACTION_PERFORMED: return result
    ...
    def heal_target_in_respect_of_skills(self, target) -> Generator[Any, Any, BehaviorResult]:
        if GLOBAL_CACHE.Agent.IsConditioned(target) and custom_behavior_helpers.Resources.is_spirit_exist(within_range=Range.Earshot):
            result = yield from custom_behavior_helpers.cast_skill_to_target(self.mend_body_and_soul, target_agent_id=target)
            if result is BehaviorResult.ACTION_PERFORMED: return result
        elif custom_behavior_helpers.Resources.is_spirit_exist(within_range=Range.Earshot):
            result = yield from custom_behavior_helpers.cast_skill_to_target(self.spirit_light, target_agent_id=target)
            if result is BehaviorResult.ACTION_PERFORMED: return result
        elif custom_behavior_helpers.Resources.is_player_holding_an_item():
            result = yield from custom_behavior_helpers.cast_skill_to_target(self.soothing_memories, target_agent_id=target)
            if result is BehaviorResult.ACTION_PERFORMED: return result
        elif not custom_behavior_helpers.Resources.is_player_holding_an_item():
            result = yield from custom_behavior_helpers.cast_skill(self.protective_was_kaolai)
            if result is BehaviorResult.ACTION_PERFORMED: return result
    ...
    def heal_target_emergency(self, target) -> Generator[Any, Any, BehaviorResult]:
        ...
        result = yield from custom_behavior_helpers.cast_skill_to_target(self.breath_of_the_great_dwarf, target_agent_id=target)
        if result is BehaviorResult.ACTION_PERFORMED: return result

        result = yield from custom_behavior_helpers.cast_skill_to_target(self.protective_was_kaolai, target_agent_id=target)
        if result is BehaviorResult.ACTION_PERFORMED: return result
    ...
    def heal_group(self) -> Generator[Any, Any, BehaviorResult]:
        ...
        result = yield from custom_behavior_helpers.cast_skill(self.breath_of_the_great_dwarf)
        if result is BehaviorResult.ACTION_PERFORMED: return result
