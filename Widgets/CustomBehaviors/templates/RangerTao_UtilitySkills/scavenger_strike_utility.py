...
class ScavengerStrikeUtility(CustomSkillUtilityBase):
    def __init__(self, skill: CustomSkill, current_build: list[CustomSkill], score_definition: ScoreStaticDefinition, mana_required_to_cast: int):
    ...
    def _evaluate(self, current_state: BehaviorState, previously_attempted_skills: list[CustomSkill]) -> float | None:
        ...
        # Check if we have a valid target with conditions and >= 50% health
        target = custom_behavior_helpers.Targets.get_first_or_default_from_enemy_ordered_by_priority(
            within_range=Range.Spellcast,
            condition=lambda agent_id: GLOBAL_CACHE.Agent.IsConditioned(agent_id) and GLOBAL_CACHE.Agent.GetHealth(agent_id) >= 0.50,
            sort_key=(TargetingOrder.DISTANCE_ASC, TargetingOrder.HP_ASC)
        )

        if target is None: return None
        return self.score_definition.get_score()  # High priority for conditioned targets
