...
from HeroAI import types, globals, constants, utils, candidates, players, game_option, windows, targeting, combat, cache_data
...
#
# import HeroAI.custom_skill as CustomSkillClassX
# importlib.reload(CustomSkillClassX)
#
# import Py4GWCoreLib.SkillManager as SkillManager
# importlib.reload(SkillManager)
...
# importlib.reload(HeroAI.custom_behavior)




# custom = HeroAI.custom_behavior.SkillManager()

MODULE_NAME = "HeroAI"
MELEE_RANGE_VALUE = constants.MELEE_RANGE_VALUE
RANGED_RANGE_VALUE = constants.RANGED_RANGE_VALUE
FOLLOW_DISTANCE_OUT_OF_COMBAT = constants.FOLLOW_DISTANCE_OUT_OF_COMBAT
...
def Follow(cached_data: CacheData):
    ...
    close_distance_check = (utils.DistanceFromWaypoint(follow_x, follow_y) <= follow_distance)
    ...
    angle_on_hero_grid = follow_angle + Utils.DegToRad(globals.hero_formation[hero_grid_pos])
        ...
        match selected_tab:
            case TabType.control_panel:
                ...
                if own_party_number == 0:
                    ...
                    game_option = windows.DrawPanelButtons(cached_data.HeroAI_vars.global_control_game_struct)
                    windows.CompareAndSubmitGameOptions(cached_data, game_option)
                        ...
                        for index in range(MAX_NUM_PLAYERS):
                            if cached_data.HeroAI_vars.all_player_struct[index].IsActive and not cached_data.HeroAI_vars.all_player_struct[index].IsHero:
                                ...
                                if PyImGui.tree_node(f"{player_name}##ControlPlayer{index}"):
                                    game_option = windows.DrawPanelButtons(original_game_option)
                                    windows.SubmitGameOptions(cached_data, index, game_option, original_game_option)
                                    PyImGui.tree_pop()
                else:
                    # follower control panel
                    original_game_option = cached_data.HeroAI_vars.all_game_option_struct[own_party_number]
                    game_option = windows.DrawPanelButtons(original_game_option)
                    windows.SubmitGameOptions(cached_data, own_party_number, game_option, original_game_option)
            case TabType.candidates:
                windows.DrawCandidateWindow(cached_data)
            case TabType.flagging:
                windows.DrawFlaggingWindow(cached_data)
            case TabType.config:
                windows.DrawOptions(cached_data)
...
def DrawEmbeddedWindow(cached_data: CacheData):
    global selected_tab
    parent_frame_id = UIManager.GetFrameIDByHash(constants.PARTY_WINDOW_HASH)
    outpost_content_frame_id = UIManager.GetChildFrameID(constants.PARTY_WINDOW_HASH, constants.PARTY_WINDOW_FRAME_OUTPOST_OFFSETS)
    explorable_content_frame_id = UIManager.GetChildFrameID(constants.PARTY_WINDOW_HASH, constants.PARTY_WINDOW_FRAME_EXPLORABLE_OFFSETS)
...
def UpdateStatus(cached_data: CacheData):
    ...
    candidates.RegisterCandidate(cached_data)
    candidates.UpdateCandidates(cached_data)
    candidates.ProcessCandidateCommands(cached_data)
    players.RegisterPlayer(cached_data)
    players.RegisterHeroes(cached_data)
    players.UpdatePlayers(cached_data)
    game_option.UpdateGameOptions(cached_data)

    cached_data.UpdateGameOptions()

    DrawEmbeddedWindow(cached_data)
    if cached_data.ui_state_data.show_classic_controls:
        windows.DrawMainWindow(cached_data)
        windows.DrawControlPanelWindow(cached_data)
        windows.DrawMultiboxTools(cached_data)
    ...
    windows.DrawFlags(cached_data)

    draw_Targeting_floating_buttons(cached_data)

    if (
            not cached_data.data.player_is_alive or
            utils.DistanceFromLeader(cached_data) >= Range.SafeCompass.value or
